...
class CountryAdapter(private val shuCountry: ArrayList<Character>,
    ...
    private val shuRvSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (shuCountry.size/3 + 1) * 205.toFloat(), mContext.resources.displayMetrics).toInt()
    private val wuRvSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (wuCountry.size/3 + 1) * 205.toFloat(), mContext.resources.displayMetrics).toInt()
    private val weiRvSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (weiCountry.size/3 + 1) * 205.toFloat(), mContext.resources.displayMetrics).toInt()
    private val kingdomlessRvSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (kingdomlessCountry.size/3) * 205.toFloat(), mContext.resources.displayMetrics).toInt()
